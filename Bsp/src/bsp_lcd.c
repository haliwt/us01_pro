...
void LCD_disp_FiveSixSeveEight_Init(void)
{
   ...
   TM1723_Write_Display_Data(0xCB,(COLON_SYMBOL+lcdNumber6_Low[glcd_t.number6_low]+lcdNumber7_High[glcd_t.number7_high]) & 0xff);
    ...
    TM1723_Write_Display_Data(0xCE,lcdNumber8_Low[glcd_t.number8_low]+WIND_SPEED_FULL);//display "t,c"
      ...
      if(glcd_t.gTimer_fan_blink > 20){//15
               glcd_t.gTimer_fan_blink =0;

               fan_blink_full ++;
            glcd_t.number5_low = 0 ;   //gpro_t.disp_works_hours_value,gpro_t.disp_works_minutes_value
        	...
        	glcd_t.number8_high = glcd_t.number8_low ;
             ...
             if(fan_blink_full ==1){
                 ...
                 TM1723_Write_Display_Data(0xCA,(lcdNumber5_Low[glcd_t.number5_low]+lcdNumber6_High[glcd_t.number6_high]) );
               //odd number
                TM1723_Write_Display_Data(0xC9,(HUM_T8+lcdNumber4_Low[glcd_t.number4_low]+lcdNumber5_High[glcd_t.number5_high]));
                //TM1723_Write_Display_Data(0xCA,(lcdNumber5_Low[glcd_t.number5_low]+lcdNumber6_High[glcd_t.number6_high]) );
...
void Set_LCD_Timer_Numbers_FiveSixSeveEight_Hours(void)
{
    ...
    static uint8_t alternate_flag,colon_flag_toggle;
   
    static uint8_t  set_timer_fan;
    ...
    if(gctl_t.ptc_warning ==1 || gctl_t.fan_warning ==1){
       ...
       switch(wifi_t.set_wind_speed_value){
        ...
        if(glcd_t.gTimer_fan_blink > 20){

             glcd_t.gTimer_fan_blink =0;
             set_timer_fan++;
             
             if(set_timer_fan == 1){
              if(gkey_t.key_mode == mode_set_timer){//if(gkey_t.key_mode == mode_set_timer && gpro_t.gTimer_timer_led_blink < 2){
    ...
    case 1:
      ...
      if(glcd_t.gTimer_fan_blink > 20){
        ...
        if(set_timer_fan ==1 && gkey_t.key_mode == mode_set_timer){
        ...
        else if(gkey_t.key_mode == mode_set_timer){ //close
 ...
 * 
*****************************************************************************/
void LCD_Timer_Colon_Flicker(uint8_t sel_item)
{
  #if 0
   switch(sel_item){
   ...
   #endif 
 if(gkey_t.key_mode != mode_set_timer || (gctl_t.ptc_warning ==0 && gctl_t.fan_warning ==0)){
...
/**************************************************************************************************
*
*Function Name:void fan_default_warning_fun(void)
